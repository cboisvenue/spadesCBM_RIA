...
times <- list(start = 1985.00, end = 2015.00)
parameters <- list(
  CBM_defaults = list(
    .useCache = TRUE
  ),
...
#whens <- sort(c(times$start, times$start + 1:4 * 100, times$end - 2:0))
# this is for the presentDay runs
whens <- sort(c(times$start, times$start + c(5, 10, 15, 20, 25), times$end - 2:0))
outputs <- as.data.frame(expand.grid(objectName = c("cbmPools", "NPP"), saveTime = whens))
 
spadesCBMSim2 <- simInit(times = times,
                        params = parameters,
                        modules = modules, 
                        objects = objects, 
                        outputs = outputs,
...
spadesCBMout2 <- spades(spadesCBMSim2, debug = TRUE)
```
